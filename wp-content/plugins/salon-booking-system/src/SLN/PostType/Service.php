...
class SLN_PostType_Service extends SLN_PostType_Abstract
{
    ...
    public function manage_columns($columns)
    {
        $new_columns = array(
            'cb' => $columns['cb'],
            'title' => $columns['title'],
            'service_duration' => __('Durata', 'salon-booking-system'),
            'service_price' => __('Pret', 'salon-booking-system'),
            'taxonomy-sln_service_category' => $columns['taxonomy-sln_service_category'],
            'sln_days_off' => __('Disponibilitate', 'salon-booking-system'),
        );
...
//        );
    ...
    public function enter_title_here($title, $post)
    {
        ...
        if ($this->getPostType() === $post->post_type) {
            $title = __('Scrie denumirea serviciului', 'salon-booking-system');
        }
    ...
    public function updated_messages($messages)
    {
        ...
        $messages[$this->getPostType()] = array(
            ...
            1 => sprintf(
                __('Actualizarea serviciulu.', 'salon-booking-system')
            ),
            2 => '',
            3 => '',
            4 => __('Actualizarea serviciului.', 'salon-booking-system'),
            5 => isset($_GET['revision']) ? sprintf(
                __('Serviciu restabilit %s', 'salon-booking-system'),
                wp_post_revision_title((int)$_GET['revision'], false)
            ) : false,
            6 => sprintf(
                __('Serviciu publicat.', 'salon-booking-system')
            ),
            7 => __('Serviciu salvat.', 'salon-booking-system'),
            8 => sprintf(
                __('Serviciu transmis.', 'salon-booking-system')
            ),
            9 => sprintf(
                __(
            ...
            10 => sprintf(
                __('Proiectul de serviciu actualizat.', 'salon-booking-system')
            ),
        );
