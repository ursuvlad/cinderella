        ...
        foreach ($bookingServices->getItems() as $bookingService) {
            ...
            if ($isMultipleAttSelection) {
                ...
                if (empty($tmp)) {
                    $this->addError(
                        sprintf(
                            __('Nici unul dintre insotitor nu este disponibil pentru %s serviciu', 'salon-booking-system'),
                            $service->getName()
                        )
                    );
                    return false;
                ...
                elseif (isset($values['attendants'][$service->getId()])) {
                    if (!in_array($values['attendants'][$service->getId()], $availAttsForEachService[$service->getId()])) {
                        $this->addError(
                            sprintf(
                                __('Insotitorul %s pentru serviciul %s nu este disponibil la %s', 'salon-booking-system'),
                ...
                if (empty($availAtts)) {
                    $this->addError(__('Nici unul dintre insotitor nu este disponibil', 'salon-booking-system'));
                    return false;
                }
