...
class SLN_GoogleScope {
            ...
            jQuery(document).ready(function () {
                jQuery('#sln_synch').click(function () {
                    ...
                    jQuery.post(ajaxurl, data, function (response) {
                        if (response == 'OK') {
                            alert("<?php echo __('Operatiune completa!', 'salon-booking-system'); ?>");
                        } else {
                            var tmp = data.split('|');
                            if (tmp[1])
                                alert(tmp[1]);
                ...
                jQuery('#sln_del').click(function () {
                    ...
                    jQuery.post(ajaxurl, data, function (response) {
                        if (response == 'OK') {
                            alert("<?php echo __('Operatiune finisata!', 'salon-booking-system'); ?>");
                        } else {
                            var tmp = data.split('|');
                            if (tmp[1])
                                alert(tmp[1]);
    ...
    public function start_synch() {
        if (!$this->is_connected()) {
            echo "KO|" . __("Google Client nu este conectat!");
        }
